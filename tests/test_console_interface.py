...
def ui_instance():
    ...
    with (
        patch("src.ui_interfaces.console_interface.HeadHunterAPI"),
...
def test_show_vacancies_for_deletion_invalid_input(ui_instance):
    ...
    test_vacancies = [Vacancy(vacancy_id="1", title="Python Developer", url="https://example.com/java-developer")]
...
def test_delete_saved_vacancies_by_keyword_advanced(ui_instance):
    ...
    with patch("src.ui_interfaces.console_interface.get_user_input", return_value="python"):
        with patch(
            "src.ui_interfaces.console_interface.filter_vacancies_by_keyword", return_value=[test_vacancies[0]]
        ):
