...
class TestVacancyDisplayHandler:
    ...
    def test_search_saved_vacancies_by_keyword_no_matches(self, mock_print, mock_input, mock_filter, handler):
        ...
        mock_print.assert_called_once_with(
            "Среди сохраненных вакансий не найдено ни одной с ключевым словом 'python'."
        )
    ...
    @patch("builtins.print")
    def test_search_saved_vacancies_by_keyword_success(
        self, mock_print, mock_input, mock_filter, mock_paginate, handler
    ):
    ...
    @patch("src.ui_interfaces.vacancy_display_handler.logger")
    @patch("builtins.print")
    def test_show_top_vacancies_by_salary_logger_exception_coverage(
        self, mock_print, mock_logger, mock_input, handler
    ):
    ...
    @patch("src.ui_interfaces.vacancy_display_handler.logger")
    @patch("builtins.print")
    def test_search_saved_vacancies_by_keyword_logger_exception_coverage(
        self, mock_print, mock_logger, mock_input, handler
    ):
